...
## 2026-03-25 feiling 正式待机路线调整

独立 `feiling` 项目当前的正式待机方案已调整为：

1. 固定绯铃母版与状态图
2. 接收透明背景 sprite sheet 原始稿
3. 用本地脚本切分为统一裁切的连续 PNG 帧
4. 在 WPF 桌宠里按序循环播放

这意味着：

- `feiling` 当前正式待机已经不再优先依赖旧的 blink overlay 实验
- “AI 一帧一帧画微动作”不再是主路线
- 当前更现实的主线是：
  - `sprite sheet -> 连续 PNG -> WPF 循环`

### 运行时实现注意点

这条路线当前还额外固化了一个重要经验：

- 如果原始 `idle_loop` 帧已经统一按共享 alpha 边界裁切
- 运行时就不能再对每一帧单独按透明边界做二次裁切
...
- `idle_loop` 帧直接按原尺寸读取播放
- 不再对单帧重复调用透明边界裁切

并且进一步确认：

- 这组 idle sprite frames 更适合“乒乓循环”
- 不适合直接首尾闭环
...
## 2026-03-25 feiling 移动行为进入第一轮实现

独立 `feiling` 项目当前的实现重点开始从：
...
2. 模式互斥
3. 手动拖拽优先级高于自动移动
4. 闲逛按“走走停停”而不是连续巡逻实现

这意味着后续 `feiling` 的产品线开始进入：
